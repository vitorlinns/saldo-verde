...
export default function EntrancePage() {
  ...
  useEffect(() => {
    try {
      ...
      console.error('EntrancePage init error:', err);
      navigate('/login', { replace: true });
    }
  }, [navigate]);
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const record: RecordItem = {
      type: 'income',
      title: title || 'Entrada sem título',
      category,
      ...
      <div className="min-h-screen h-full grid w-full gap-6 lg:grid-cols-[280px_1fr] lg:items-stretch">
        ...
        <div className="mr-4 flex min-h-screen flex-col">
          ...
          <section className="flex-1 space-y-6">
            <div>
              <h1 className="text-3xl font-semibold text-white">Registrar nova entrada</h1>
            ...
            <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
              <div className="rounded-xl border border-border bg-black/95 p-4 shadow-xl shadow-black/20 sm:p-6">
                <form className="space-y-4" onSubmit={handleSubmit}>
                  {successMessage ? (
                    <div className="rounded-2xl border border-success bg-success/10 p-3 text-sm text-success">
                      {successMessage}
                    </div>
                  ) : null}
