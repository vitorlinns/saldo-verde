    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-lg rounded-[0.5rem] border border-border bg-surface p-6 text-white">
        <div className="space-y-4">
          ...
          {children ? <div className="rounded-[0.5rem] border border-border bg-surface p-4 text-base text-white/80">{children}</div> : null}

          {warning ? (
            <div className="rounded-[0.5rem] border border-danger bg-danger_bg/10 p-4 text-sm text-danger">
              {warning}
            </div>
          ) : null}
        </div>
