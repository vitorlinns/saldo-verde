...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    try {
      ...
      console.error('[auth/login] setAuthCookies error:', cookieError);
      return sendJson(res, 200, {
        session: data.session,
        user: data.user,
        message: 'Login realizado com sucesso.',
        warning: 'Sessão por cookie indisponível no momento.',
      });
    }
    ...
    console.error('[auth/login] unhandled error:', unhandledError);
